#'---
#' title: "PASS1B (Rat) RNA-Seq: EDA and Processing"
...
pacs...man <- c("tidyverse","GenomicRanges", "DESeq2","devtools","rafalib","GO.db","vsn","hexbin","ggplot2",
                ...
                "PROPER","SeqGSEA",'purrr','BioInstaller','RColorBrewer','lubridate', "hms","ggpubr", "ggrepel","reshape2",'data.table','MotrpacBicQC')
...
counts_1 <- read.table(file = paste0(WD,'/transcriptomics/',tissue,'/transcript-rna-seq/results/motrpac_pass1b-06_',tissue,'_transcript-rna-seq_rsem-genes-count.txt'), 
                   header = TRUE, sep = '\t', check.names = FALSE)

# Tissue meta data
meta_1 <-  fread(file = paste0(WD,'/phenotype/motrpac_pass1b-06_pheno_viallabel-data.txt'), header = TRUE, sep = '\t') %>% as_tibble()
...
colnames(meta_1) = new_names

# TODO: Nicole adjusts the nomenclature of certain variables for readability, consider investigating these formats independently and if new features discovered, share with Nicole.
# var <- 'key.agegroup'
# for (var in c('key.protocol','key.agegroup','key.intervention','key.sacrificetime','registration.sex')){
#         d = cols_1 %>%
#                 filter(Field.Name == gsub('.*\\.','',var))
...
meta_1$Key.sacrificetime %>% table()
meta_1$Key.d_sacrifice  %>% table()
meta_1$Key.intervention %>% table()
