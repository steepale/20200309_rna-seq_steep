#'---
#' title: "PASS1A (Rat) RNA-Seq: Combine Data and Process Counts"
#' author: "Alec Steep" 
#' date: "20200309"
#' output:
#'     html_document:
...
pacs...man <- c("tidyverse","GenomicRanges", "DESeq2","devtools","rafalib","GO.db","vsn","hexbin","ggplot2",
                ...
                "org.Rn.eg.db","pheatmap","sva","formula.tools","pathview","biomaRt",
...
#+ Create Tisse Annotation

################################################################################
#####     Create Tisse Annotation      #########################################
################################################################################

# Create a tissue annotation dataframe
Folder <- c('t55-gastrocnemius','t61-colon','t66-lung','t30-blood-rna','t56-vastus-lateralis','t62-spleen','t67-small-intestine','t52-hippocampus','t58-heart','t63-testes','t68-liver','t53-cortex','t59-kidney','t64-ovaries','t69-brown-adipose','t54-hypothalamus','t60-adrenal','t65-aorta','t70-white-adipose')
Tissue <- c('Gastrocnemius','Colon','Lung','Blood','Vastus Lateralis','Spleen','Small Intestine','Hippocampus','Heart','Testes','Liver','Cortex','Kidney','Ovaries','Brown Adipose','Hypothalamus','Adrenal','Aorta','White Adipose')
Code <- c('SKM','CLN','LUNG','BLD','VLS','SPL','SMI','HIP','HAT','TES','LIV','COR','KID','OVR','BAT','SCN','ADG','AOR','WAT')
tis_ann_df <- data.frame(Tissue,Code,Folder) %>%
        arrange(Tissue)

# Save the file
tis_ann_file <- paste0(WD, '/data/20200309_pass1a-rnaseq-tissue-annotation_steep.txt')
#write.table(tis_ann_df, file = tis_ann_file, quote = F, sep = '\t', row.names = F)
...
# Collect gene ids
folder <- "t60-adrenal"
raw_count_file <- paste0("/Volumes/Frishman_4TB/motrpac/20200630_pass1a-rnaseq_2.0/",folder,"/transcript-rna-seq/results/motrpac_pass1a-06_",folder,"_transcript-rna-seq_rsem-genes-count.txt")
Tissue <- tis_ann_df %>% filter(Folder == folder) %>% select(Tissue) %>% unlist() %>% as.character()
tissue_list[[Tissue]] <- read.table(raw_count_file, sep = '\t', header = T, check.names = F)
ENSEMBL_RAT <- tissue_list[[Tissue]][,1]

counts_df <- data.frame(ENSEMBL_RAT)
tissue_list <- list()
for(folder in tis_ann_df$Folder){
        raw_count_file <- paste0("/Volumes/Frishman_4TB/motrpac/20200630_pass1a-rnaseq_2.0/",folder,"/transcript-rna-seq/results/motrpac_pass1a-06_",folder,"_transcript-rna-seq_rsem-genes-count.txt")
        Tissue <- tis_ann_df %>% filter(Folder == folder) %>% select(Tissue) %>% unlist() %>% as.character()
        tissue_list[[Tissue]] <- read.table(raw_count_file, sep = '\t', header = T, check.names = F)
        print(Tissue)
        counts_df <- cbind(counts_df, tissue_list[[Tissue]][,-1])
}

# All rows are equal
# for(i in 1:length(tissue_list)){
#         n = 1
#         print(all(row.names(tissue_list[[n]]) == row.names(tissue_list[[i]])))
# }

# Save the counts file
counts_file <- paste0(WD, '/data/20200309_pass1a-rnaseq-all-tissues-rsem-genes-count.txt')
#write.table(counts_df, file = counts_file, quote = F, sep = '\t', row.names = F)

# Load the metadata
metadata_file <- '/Volumes/Frishman_4TB/motrpac/20200630_pass1a-rnaseq_2.0/qa-qc/motrpac_pass1a-06_transcript-rna-seq_qa-qc-metrics.csv'
meta_df <- read.table(metadata_file, sep = ',', header = T, check.names = F)
meta_df %>% arrange(Tissue) %>%
        filter(vial_label %in% head(names(counts_df)))
