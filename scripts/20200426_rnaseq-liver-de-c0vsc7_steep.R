#'---
#' title: "PASS1A Rat Liver: -- Control 0 hr vs Control 7 hr Differential Gene Expression"
...
#' * Determine if DE gene list is enriched for circadian genes in the liver
...
in_file <- paste0(WD,'/data/20200503_rnaseq-circadian-LIV-mouse-rat-ortho_steep-yan.txt')
...
#' ##### Female liver Samples (n unique) from 1 batch were filtered prior to normalization
...
tis_counts <- count_data[,tis] %>% as.matrix()
...
tis_cols$controls_binary <- factor(tis_cols$controls_binary, 
...
dds1 <- DESeqDataSetFromMatrix(countData = tis_counts,
                               colData = tis_cols,
                               design = design)
# Reasoning from:
#citation("PROPER")
...
in_file <- paste0(WD,'/data/20200505_rnaseq-bothsexes-liver-phases_steep.txt')
...
phase_df <- left_join(de_df, sinmod_df, by ="ENSEMBL_RAT") %>%
        ...
        mutate(CIRC = as.factor(ifelse(ENSEMBL_RAT %in% circ_df$ENSEMBL_RAT, 'CIRC', 'NON-CIRC')))
...
circ_genes <- ENSEMBL_RAT[ENSEMBL_RAT %in% circ_df$ENSEMBL_RAT]
...
fkid <- col_data %>%
        ...
        select(sample_key) %>% unlist() %>% as.character()
tis_counts <- count_data[,fkid] %>% as.matrix()
...
tis_cols$animal.key.anirandgroup <- factor(tis_cols$animal.key.anirandgroup,
                                            levels = ec_levels)
row.names(tis_cols) <- tis_cols$sample_key
#' #### Sanity Check: Ensure that the metadata rownames are identical to count matrix column names
all(rownames(tis_cols) == colnames(tis_counts))
...
rld_circ <- rld_medcen[row.names(rld_medcen) %in% circ_df$ENSEMBL_RAT,]
mypar()
...
# Make sure the "control" or "untreated" level is first
tis_cols$C_compare <- factor(tis_cols$C_compare, 
